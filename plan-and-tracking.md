...
| 5 | Domain | Subdomain `portofolio.rezagiovanni.my.id` → bucket (CNAME di Hostinger ✅, site LIVE via HTTP ✅). **SSL GCS pending** (cron monitor tiap 2j). Foto fix: relative `/original/` path. | Cloud DNS | 🔄 |
